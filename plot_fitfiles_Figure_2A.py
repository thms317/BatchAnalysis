...
for n, fitfile in enumerate(fitfiles):
    ...
    if p['NRL'] == 167:
        ...
        plt.scatter(z_pull, f_pull, color='blue', label=str(int(p['NRL'])), s=30, zorder=25,
                    facecolors='none')
        # plt.scatter(z_pull, f_pull, color=next(colors), label=str(int(p['NRL'])) + " " + fitfile[:-4], s=30, zorder=30,
        #             facecolors='none')
